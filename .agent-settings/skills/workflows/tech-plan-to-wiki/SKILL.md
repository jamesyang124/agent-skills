...
```bash
# Claude
mkdir -p ~/.claude/skills/tech-plan-to-wiki
cp <agent-settings-repo>/.agent-settings/skills/workflows/tech-plan-to-wiki/SKILL.md \
   ~/.claude/skills/tech-plan-to-wiki/SKILL.md
# Add to ~/.claude/CLAUDE.md: - **tech-plan-to-wiki** (`~/.claude/skills/tech-plan-to-wiki/SKILL.md`)
...
```bash
install-atlassian-mcp skill (run it in your agent)
```
...
> **Before proceeding**: Check `.agent-settings/project-config.md` for the `## SDD Tool` section. Use the configured tool name (e.g., `spec-kit` or `openspec`) in all user-facing messages and artifact location hints. Fall back to `spec-kit` if the section is absent or the file does not exist.

### Step 1: Locate SDD Artifacts
...
- If none are found, ask the user to provide the path to the {configured SDD tool} project directory.
...
```
I've read the SDD artifacts and mapped them to a design review page.
...
Next steps:
  ...
  4. Re-run /tech-plan-to-wiki to publish the updated version.
  5. Repeat until team reaches consensus and status is Approved (v1).

📌 Save your page ID for future re-runs: XXXXXXXXX
   You can pass it directly next time: /tech-plan-to-wiki [page-id]
```
...
- **Approved (v1)**: Update when team reaches consensus and plan is finalized.
  - After `Approved (v1)`, the plan is ready for `/tech-plan-to-ticket`.
...
| All files missing | Stop. Ask user to provide the {configured SDD tool} project directory path. |
...
- **Link back to source**: In the page footer or metadata, note which {configured SDD tool} directory the content came from (if available) so the team knows where the local files live.
