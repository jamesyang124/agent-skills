...
Before creating any Jira tickets, present all derived scenarios to the RD using the format defined in `.agent-settings/skills/workflows/sdd-qa-to-jira/references/qa-workflow.md` → "Step 5: Scenario Presentation Template".
...
Before presenting scenarios to the RD, verify each one against the checklist in `.agent-settings/skills/workflows/sdd-qa-to-jira/references/qa-workflow.md`. Drop any scenario that fails — do not create low-quality tickets.
...
See `.agent-settings/skills/workflows/sdd-qa-to-jira/references/qa-workflow.md` for the workflow position diagram (Implement → QA → Release).
